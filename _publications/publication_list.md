...
<span style=""><em>24th International Symposium on Communications and Information Technologies (ISCIT)</em></span>, 2025.
</li>

<li>
A. Zaker, <strong>N. T. Nguyen</strong>, A. Alkhateeb, and M. Juntti,  
"<a href="" target="_blank">Constrained Multimodal Sensing-Aided Communications: A Dynamic Beamforming Design</a>,"  
<span style=""><em>IEEE Global Communications Conference (GLOBECOM)</em></span>, 2025.
</li>
...
<span style=""><em>Asilomar Conference on Signals, Systems, and Computers (ASILOMAR)</em></span>, 2025.
</li>
...
<span style=""><em>International Symposium on Antennas and Propagation (ISAP)</em></span>, 2025.
</li>
...
<span style=""><em>IEEE Workshop on Signal Processing Advances in Wireless Communications (SPAWC)</em></span>, 2025.
</li>
...
<span style=""><em>IEEE International Conference on Acoustics, Speech, and Signal Processing (ICASSP)</em></span>, 2025.
</li>
...
<span style=""><em>European Conference on Antennas and Propagation (EuCAP)</em></span>, 2025.
</li>
...
<span style=""><em>IEEE JC&S Symposium</em></span>, 2024.
</li>
...
<span style=""><em>International Workshop on Energy-Aware Mobile IoT (Eware-IoT)</em></span>, 2024.
</li>
...
<span style=""><em>IEEE Workshop on Signal Processing Advances in Wireless Communications (SPAWC)</em></span>, Sept. 2024, Lucca, Italy.
</li>
...
<span style=""><em>IEEE International Conference on Communications and Electronics (ICCE)</em></span>, Aug. 2024, Da Nang City, Vietnam.
</li>
...
<span style=""><em>IEEE International Mediterranean Conference on Communications and Networking (MEDITCOM)</em></span>, July 2024, Madrid, Spain.
</li>
...
<span style=""><em>IEEE Radar Conference (RadarConf24)</em></span>, 2024, Denver, CO, USA.
</li>
...
<span style=""><em>IEEE Wireless Communications and Networking Conference (WCNC)</em></span>, 2024.
</li>
...
<span style=""><em>IEEE Global Communications Conference (GLOBECOM)</em></span>, 2023, Kuala Lumpur, Malaysia.
</li>
...
<span style=""><em>Asilomar Conference on Signals, Systems, and Computers (ASILOMAR)</em></span>, 2023, Pacific Grove, CA, USA. (accepted)
</li>
...
<span style=""><em>IEEE Workshop on Signal Processing Advances in Wireless Communications (SPAWC)</em></span>, Sept. 2023, Shanghai, China. (Best Student Paper Award)
</li>
...
<span style=""><em>IEEE Statistical Signal Processing Workshop (SSP)</em></span>, July 2023, Hanoi, Vietnam. (Best Paper Award)
</li>
...
<span style=""><em>IEEE International Conference on Acoustics, Speech, and Signal Processing (ICASSP)</em></span>, June 2023, Rhodes Island, Greece.
</li>
...
<span style=""><em>IEEE International Conference on Communications (ICC)</em></span>, May 2023, Rome, Italy.
</li>
...
<span style=""><em>IEEE Global Communications Conference (GLOBECOM) Workshop</em></span>, Dec. 2022, Rio de Janeiro, Brazil.
</li>
...
<span style=""><em>IEEE Global Communications Conference (GLOBECOM)</em></span>, Dec. 2022, Rio de Janeiro, Brazil.
</li>
...
<span style=""><em>IEEE Workshop on Signal Processing Advances in Wireless Communications (SPAWC)</em></span>, July 2022, Oulu, Finland.
</li>
...
<span style=""><em>IEEE International Conference on Communications (ICC)</em></span>, May 2022, Seoul, South Korea.
</li>
...
<span style=""><em>IEEE Vehicular Technology Conference (VTC Spring)</em></span>, June 2022, Helsinki, Finland.
</li>
...
<span style=""><em>IEEE Global Communications Conference (GLOBECOM)</em></span>, Dec. 2021, Madrid, Spain.
</li>
...
<span style=""><em>IEEE International ITG Workshop on Smart Antennas (WSA)</em></span>, Nov. 2021, French Riviera, France.
</li>
...
<span style=""><em>IEEE Vehicular Technology Conference (VTC Fall)</em></span>, Sept. 2021, Norman, OK, USA.
</li>
...
<span style=""><em>International Conference on Advanced Technologies for Communications (ATC)</em></span>, Oct. 2021, Hanoi, Vietnam. (Best Paper Award)
</li>
...
<span style=""><em>IEEE Workshop on Signal Processing Advances in Wireless Communications (SPAWC)</em></span>, Sept. 2021, Lucca, Italy.
</li>
...
<span style=""><em>EuCNC & 6G Summit</em></span>, June 2021, Grenoble, France.
</li>
...
<span style=""><em>IEEE International Conference on Communications (ICC) Workshop</em></span>, June 2021, Montreal, Canada.
</li>
...
<script>
(function () {
  // --- CONFIG: tweak if your structure changes ---
  const YEAR_RX = /(20\d{2})/; // find a 4-digit year like 2024/2025
  const isJournalLike = (venue) =>
    /journal|transactions|letters|magazine|proceedings of the ieee/i.test(venue);

  // Helper: HTML encode
  const enc = (s) => String(s || "").replace(/[&<>"']/g, m => ({
    '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'
  }[m]));

  // Helper: simple BibTeX key: FirstAuthorLastName + Nguyen + Year if present
  function makeBibKey(authors, title, year) {
    const parts = authors.split(/,|\band\b/).map(s => s.trim()).filter(Boolean);
    let first = parts[0] || "Author";
    // Pull last name from "H. T. Nguyen" -> "Nguyen"; "Ma, M." -> "Ma"
    if (/,/.test(first)) first = first.split(",")[0].trim();
    else {
      const toks = first.split(/\s+/);
      first = toks[toks.length - 1] || first;
    }
    const core = (first || "Author").replace(/[^A-Za-z]/g, "");
    const y = year || new Date().getFullYear();
    const tword = (title || "").split(/\W+/).filter(Boolean)[0] || "Paper";
    return `${core}${tword}${y}`;
  }

  // Build the details/BibTeX HTML (matches your style)
...
<details style="display:block; margin-top:6px;">
  ...
  <div style="position:relative; margin-top:8px; background:#ffeef5; border:1px solid #f6c5db; border-radius:8px; padding:10px; text-align:left;">
    <pre style="margin:0; overflow:auto; font-size:12px; line-height:1.25;"><code id="${codeId}">${enc(bibtex)}</code></pre>
    <textarea id="${taId}" readonly style="position:absolute; left:-9999px; top:-9999px;">${enc(bibtex).replace(/&amp;|&lt;|&gt;|&quot;|&#39;/g, s => ({
      '&amp;':'&','&lt;':'<','&gt;':'>','&quot;':'"','&#39;':"'"
    }[s]))}</textarea>
    <button id="${btnId}" style="position:absolute; top:6px; right:6px; border:1px solid #c7d2e0; background:#eef2f7; border-radius:6px; padding:2px 8px; font-size:12px; cursor:pointer;"
      onclick="(function(btn){var ta=document.getElementById('${taId}');ta.select();ta.setSelectionRange(0,999999);var ok=false;try{ok=document.execCommand('copy');}catch(e){} if(!ok&&navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(ta.value).then(function(){ok=true;}).catch(function(){});} var old=btn.textContent;btn.textContent=ok?'Copied!':'Copy';setTimeout(function(){btn.textContent=old;},1200);})(this); return false;">Copy</button>
  </div>
</details>`;
  }

  // Extract structured fields from an <li>
  function parseLi(li) {
    // If a custom data-bibtex is present, just use it
    ...
    // Authors: try first text node of the <li>
    let authors = "";
    for (const n of li.childNodes) {
      ...
      // Sometimes authors are wrapped in <span> then a <br>
  ...
  // Assemble BibTeX string
  function toBib({ authors, title, venue, year, url, status }) {
    // Convert "A. Zaker, N. T. Nguyen, ..." to BibTeX author list: "Zaker, A. and Nguyen, N. T. and ..."
    // We’ll leave as-is to keep your original style (Surname, Initials already appear in many items).
    const type = isJournalLike(venue) ? 'article' : 'inproceedings';
    const key = makeBibKey(authors || "Author", title || "Title", year || "");
    const fields = [];
    fields.push(`author = {${authors || "Unknown"}}`);
    fields.push(`title = {${title || "Untitled"}}`);
    if (type === 'article') fields.push(`journal = {${venue || "Journal"}}`);
    else fields.push(`booktitle = {${venue || "Proceedings"}}`);
    if (year) fields.push(`year = {${year}}`);
    if (status) fields.push(`note = {${status}}`);
    if (url) fields.push(`url = {${url}}`);
    return `@${type}{${key},\n  ${fields.join(",\n  ")}\n}`;
  }

  function enhanceList(liList) {
    let idx = 1;
    liList.forEach(li => {
      // Skip if BibTeX details already exist
      if (li.querySelector('details')) return;

      const parsed = parseLi(li);
      const bib = parsed.customBib ? parsed.customBib : toBib(parsed);
      const html = buildDetailsHTML(idx++, bib);

      // Insert after the title/link line: append at end is simplest & consistent
  ...
  // Run on both Journal and Conference lists (any <ol> on the page)
