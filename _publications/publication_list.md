...
<script>
(function () {
  const log = (...a) => { try { console.log('[pub-bibtex]', ...a); } catch(e) {} };
  ...
  function buildDetailsHTML(id, bibtex) {
    ...
    const rawBib = bibtex; // keep a raw copy (no HTML entities) for copying
    return `
<details style="display:block; margin-top:6px;">
  ...
  function parseLi(li) {
    ...
    for (const n of li.childNodes) {
      ...
      if (n.nodeType === Node.ELEMENT_NODE && n.tagName !== 'A' && n.tagName !== 'EM' && n.tagName !== 'DETAILS') {
        const t = (n.textContent || "").trim();
        if (t) { authors = t.replace(/[,;.\s]+$/, ''); break; }
      }
    }
  ...
  function enhance() {
    ...
    log(`found ${lis.length} <li> items`);
    if (!lis.length) {
      // If nothing found, try a slightly looser selector (some MD engines wrap in <div><ol>…)
      const allLis = Array.from(document.getElementsByTagName('li'));
      log(`fallback found ${allLis.length} <li> items`);
    }
    let idx = 1, added = 0;
    lis.forEach(li => {
      if (li.querySelector('details')) return; // already enhanced
      const parsed = parseLi(li);
      const bib = toBib(parsed);
      const holder = document.createElement('div');
      holder.innerHTML = buildDetailsHTML(idx++, bib);
      const node = holder.firstElementChild;
      if (node) { li.appendChild(node); added++; }
    });
    log(`added BibTeX panels to ${added} items`);
  }

  // Run now if possible; otherwise on DOMContentLoaded
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', enhance, { once: true });
  } else {
    try { enhance(); } catch (e) { log('error running enhance immediately:', e); }
  }
})();
</script>
{% endraw %}
